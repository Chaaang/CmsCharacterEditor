...
class CustomQRWidget extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    return Container(
      ...
      child: CustomPaint(
        painter: _CustomQRPainter(textList: textList, qrData: qrData),
      ),
    );
  }
  ...
  static Future<ui.Image> getImage({
    ...
    final painter = _CustomQRPainter(textList: textList, qrData: qrData);
...
class _CustomQRPainter extends CustomPainter {
  ...
  _CustomQRPainter({required this.textList, required this.qrData});
  ...
  void paint(Canvas canvas, Size size) {
    ...
    for (var i = 0; i < textList.length; i++) {
      ...
      textPainter.text = TextSpan(
        ...
        style: TextStyle(
          ...
          color: Colors.black,
          fontWeight: textInfo.isBold ? FontWeight.w800 : FontWeight.normal,
        ),
      );
      textPainter.layout();
    ...
    final qrPainter = QrPainter(data: qrData, version: QrVersions.auto);

    // Tamaño del código QR (ajusta esto según tus necesidades)
    final qrSize =
        size.width / 3; // Por ejemplo, un tercio del ancho del canvas
