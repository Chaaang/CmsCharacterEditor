...
class _PhotoPageState extends State<PhotoPage> {
  ...
  Future<void> _loadMask() async {
    try {
      ...
      final maskPath = widget.design.characterMask;//'assets/mask-gingerbreadman.png';
  ...
  Future<Uint8List?> _applyMask(Uint8List imageBytes) async {
    try {
      ...
      final minPixelSize = 350.0; // Minimum 350 pixels for a face to fit (was 420)
      final percentageSize = smallerDimension * 0.70; // 70% of smaller dimension (was 0.85)
      final targetSize = minPixelSize > percentageSize ? minPixelSize : percentageSize;
      ...
      final minSize = minPixelSizeForCheck > minPercentageSize ? minPixelSizeForCheck : minPercentageSize;
      ...
      if (characterId == 2 || characterId == 3) {
        ...
        final paddingAmount = size.width * 0.08; // 8% padding on each side = 16% total reduction
      ...
      final outputRect = Rect.fromLTWH(0, 0, outputWidth.toDouble(), outputHeight.toDouble());
      ...
      outputCanvas.drawImageRect(_maskImage!, maskSourceRect, outputRect, maskPaint);
...
class _MaskOverlayPainter extends CustomPainter {
  ...
  void paint(Canvas canvas, Size size) {
    ...
    final maskRectForOutline = Rect.fromLTWH(offsetX, offsetY, maskWidth, maskHeight);
    ...
    final whiteMaskPaint = Paint()
      ..colorFilter = const ColorFilter.mode(Colors.white, BlendMode.srcIn);
    canvas.drawImageRect(maskImage, maskSourceRectForOutline, maskRectForOutline, whiteMaskPaint);
    ...
    final innerRect = Rect.fromLTWH(innerOffsetX, innerOffsetY, innerWidth, innerHeight);
    final innerMaskSourceRect = Rect.fromLTWH(
      (maskImage.width.toDouble() - maskImage.width.toDouble() * scaleFactor) / 2,
      (maskImage.height.toDouble() - maskImage.height.toDouble() * scaleFactor) / 2,
      maskImage.width.toDouble() * scaleFactor,
      maskImage.height.toDouble() * scaleFactor,
    );
    
    final innerMaskPaint = Paint()
      ..blendMode = BlendMode.dstOut;
    canvas.drawImageRect(maskImage, innerMaskSourceRect, innerRect, innerMaskPaint);
    canvas.restore();
  }

  @override
  bool shouldRepaint(covariant _MaskOverlayPainter oldDelegate) =>
      oldDelegate.maskImage != maskImage || oldDelegate.characterId != characterId;
}
